...
export class OnboardingScene {
  ...
  async onText(@Ctx() ctx: BotContext) {
    ...
    if (state.step === 'phone') {
      ctx.session.onboarding = { ...state, step: 'vehicle_choice', phone: text };
    ...
    if (state.step === 'vehicle_color') {
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [Markup.button.callback('Skip for now', 'onboarding:skip_vehicle_model')],
        ]),
      );
      return;
    }
    ...
    if (state.step === 'vehicle_model') {
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [Markup.button.callback('Skip for now', 'onboarding:skip_vehicle_parking')],
        ]),
      );
      return;
    }
