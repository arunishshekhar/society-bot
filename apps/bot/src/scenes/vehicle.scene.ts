...
import { isValidVehicleNumber, normalizeVehicleNumber } from '../utils/validation';
...
export class VehicleScene {
  ...
  private isEditableField(field: string | undefined): field is 'number' | 'type' | 'color' | 'model' | 'parkingSlot' {
    return ['number', 'type', 'color', 'model', 'parkingSlot'].includes(field ?? '');
  }
