...
export class AdminController {
  ...
  @Patch('residents/:id')
  updateResident(@Param('id') id: string, @Body() body: { name?: string; flatNumber?: string; phone?: string | null; isActive?: boolean }) {
  ...
  async broadcast(@Body() body: { message: string; sentBy?: string }) {
    ...
    for (const resident of residents) {
      try {
        await this.bot.telegram.sendMessage(Number(resident.telegramId), `Society Notice\n\n${body.message}`);
        sent += 1;
      } catch {
